...
| Unattended execution | Requests the advertised native `fullAccess` mode on create and resume; an explicit Permission Mode takes precedence. `bypassPermissions` is not assumed to bypass CodeBuddy's high-risk checks. Policy acceptance is tested; destructive operations are not used for acceptance. |
