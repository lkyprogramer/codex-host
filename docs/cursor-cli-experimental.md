...
## Idle resource suspension

`CursorSession` exposes the shared [`resourceLifecycle`](harness-resource-lifecycle.md) contract, so the Host suspends an idle Cursor Thread after 60 seconds by closing its owned ACP process group and resumes it through `session/load` on the next operation. Suspension is refused as `busy` while a Turn, a configuration write or a snapshot read (which holds a replay process) is in progress, and as `unknown` for an aborted signal, a closed or faulted Session, or a created Session whose first Turn has not yet been verified in Cursor's local history. A history-only Session that was suspended resumes history-only for reads; only the next Turn upgrades it to a live Session with the model catalog. `thread release` therefore reports `resourcesReleased=true` with `proof.scope=cursor-acp-session` for a persisted idle Thread while `quiescence` stays `unknown`. The [2026-09-19 record](cursor-idle-suspend-20260919.md) lists the covered tests; real-CLI suspension and wake are not yet certified.
