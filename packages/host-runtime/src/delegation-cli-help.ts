...
The response confirms cwd and parent. Use its Thread reference with thread read, wait, send, or cancel.`,
  "delegate reconcile": `codexhost delegate reconcile <thread> [--apply true|false] [--format json|compact]
Dry-run by default. --apply true is refused for active work or UNKNOWN native side effects.`,
  "thread send": `codexhost thread send <thread> (--message <text> | --message-file <path>) [--request-id <id>] [--expected-turn <turn>] [--format json|compact]
...
THREAD_BUSY means the current Turn is still active: wait or cancel before sending. Messages are not queued.`,
  "thread cancel": `codexhost thread cancel <thread> [--expected-turn <turn>] [--format json|compact]
...
Compact output keeps task links, Harness, status, title, and workspace.`,
  "thread wait-many": `codexhost thread wait-many --targets-file <path> [--timeout-ms <n>] [--format json|compact]
Wait on many Threads until each changes or the timeout expires. Default 30000 ms, maximum 60000 ms, 0 is an immediate snapshot.
Cancelling wait-many does not cancel child Threads.`,
  "thread observe": `codexhost thread observe --targets-file <path> [--timeout-ms <n>] [--format json|compact]
Read-only client loop over wait-many. Returns once on terminal status, pending input, changed Turn, resync/error, review deadline, total timeout, or cancellation.
Ordinary progress does not return. Default total timeout 300000 ms, maximum 3600000 ms.`,
  "thread status": `codexhost thread status <thread> [--format json|compact]
Compact Thread, Turn, status, revision, cwd, and configuration. No historical bodies.`,
  "thread evidence": `codexhost thread evidence <thread> [--turn <turn>] [--item <item>] [--cursor <cursor>] [--limit <n>] [--include-output true|false] [--format json|compact]
User-visible command, tool, and file-change items. Default metadata omits output.`,
...
export const DELEGATION_HELP = `usage:
  ...
  codexhost thread wait-many --targets-file <path>
  codexhost thread observe --targets-file <path>
  codexhost thread status <thread>
  codexhost thread evidence <thread>
  codexhost thread configuration <thread>
  codexhost thread release <thread>
